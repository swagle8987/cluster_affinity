...
def run_script(cost, args):
    ...
    if check_input_trees([t1, t2]):
        if cost == "cluster_affinity":
            if args.unrooted:
                dist = compute_transfer_index(t1, t2, "Unrooted Cluste Affinity")
            else:
        ...
        if not args.cli:
            ...
            add_tree(
                t1,
                name="Source tree ({})".format(os.path.basename(args.t1)),
                layouts=[layout],
            )
            explore(
                t2,
                name="Target tree ({})".format(os.path.basename(args.t2)),
...
def cluster_matrix():
    ...
    make_matrix_image(
        matrix, args.outfile, xlabels=xlabels, ylabels=trees.keys(), cmap=cmap
    )
...
def cluster_affinity_script():
    ...
    parser.add_argument(
        "t1", help="The source tree from which the cost is to be calculated"
    )
    parser.add_argument(
        "t2", help="The target tree to which the cost is to be calculated"
    )
    ...
    parser.add_argument(
        "--unrooted",
        help="Calculates the unrooted cluster Affinity cost",
        action="store_true",
    )
